...
DriftWindow.prototype.set_t = function(t, paused) {
    var rx = this.t2px(t);
    this.$razor.style.left = rx;

    // auto-scroll
    var x = this.$el.scrollLeft;
    var w = this.$el.clientWidth;

    if(!paused && (rx < x || rx > (x+w))) {
        this.$el.scrollLeft = rx - 50;
    }
...
function tick() {
    drift_window.set_t($a.currentTime, $a.paused);
    window.requestAnimationFrame(tick);
}

var drift_window, zoombar, $a;
function init() {
  $a = document.createElement("audio");
  $a.src = "audio.wav";

  _get_json("pitch.json", function(resp) {
      var words = [];

      document.body.innerHTML = "";
      drift_window = new DriftWindow(words, resp.freq_hz, resp.p_voiced, on_seek);
      document.body.appendChild(drift_window.$el);
...
function checkStatus() {
  _get_json("status.json", function(resp) {
    if (resp.status === 'DONE') {
      init();
    } else {
